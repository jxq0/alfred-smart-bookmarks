...
class BookmarkRepo:
  ...
  def __init__(self, file_list_str, store_dir):
    ...
    for org_file in self.file_list:
      self.md5sum[org_file] = hashlib.md5(open(org_file,
                                               "rb").read()).hexdigest()
    ...
    if self.store_file.exists():
      self.store = json.load(self.store_file.open("r"),
                             object_hook=bookmark_decode)
  ...
  def update_store(self):
    ...
    if need_save_store:
      json.dump(self.store, self.store_file.open("w"), cls=BookmarkEncoder)
  ...
  def match_query(self, b, query):
    if query in b.heading_path:
      return True

    if query in pinyin.get(b.heading_path, format="strip"):
      return True

    if query in b.bookmark.url or query in b.bookmark.name:
      return True
  ...
  def fetch(self, query_list):
    ...
    for i in result_list:
      logger.debug("heading_path:%s, bookmark:%s", i.heading_path, i.bookmark)
